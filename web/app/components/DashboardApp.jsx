...
function DailyDataEditor() {
  ...
  const [bossEdits, setBossEdits] = useState({});
  const [state, setState] = useState({ saving: false, message: "" });
  useEffect(() => { if (!date && history[0]?.date) setDate(history[0].date); }, [date, history]);
  useEffect(() => { setDraft(editorDraft(history.find((item) => item.date === date) ?? null)); setMemberEdits({}); setBossEdits({}); setState({ saving: false, message: "" }); }, [date]);
  async function save(event) {
    event.preventDefault();
    setState({ saving: true, message: "" });
    try {
      const response = await fetch("/api/guild-stats", { method: "PUT", headers: { "content-type": "application/json", "x-archero-dashboard-action": "1" }, body: JSON.stringify({ date, ...numericEditorDraft(draft), expeditionName: draft.expeditionName, expeditionRank: draft.expeditionRank, memberEdits: Object.values(memberEdits), bossEdits: Object.values(bossEdits) }) });
      const payload = await response.json().catch(() => ({}));
  ...
  return <div className="admin-management daily-editor"><section className="panel daily-editor-panel"><div className="daily-editor-heading"><PanelHeading title="Daily data editor" subtitle="Correct all values from one captured day. Changes are kept in the audit history." /><button className="primary-button" type="submit" form="daily-editor-form" disabled={state.saving || !selected}>{state.saving ? "Saving…" : "Save changes"}</button></div>
    {history.length === 0 ? <p className="admin-module-empty">No daily guild snapshot is available yet.</p> : <form id="daily-editor-form" onSubmit={save}><div className="daily-editor-dates" aria-label="Available capture dates">{history.slice(0, 7).reverse().map((item) => <button type="button" className={item.date === date ? "active" : ""} onClick={() => setDate(item.date)} key={item.date}><strong>{item.date.slice(-2)}</strong><span>{new Date(`${item.date}T12:00:00Z`).toLocaleDateString("en", { weekday: "short" })}</span></button>)}</div>
      ...
      <div className="daily-editor-table"><table><thead>{tab === "members" ? <tr><th>Member</th><th>Power</th><th>Contribution</th><th>Boss attacks</th><th>Activity</th></tr> : <tr><th>Member</th><th>Boss damage</th><th>Rank</th><th>Source</th></tr>}</thead><tbody>{tab === "members" ? memberRows.map((row, index) => { const edit = memberEdits[row.playerId]; const set = (field, value) => row.playerId && setMemberEdits((current) => ({ ...current, [row.playerId]: { playerId: row.playerId, power: Number(edit?.power ?? row.power ?? 0), contribution7d: Number(edit?.contribution7d ?? row.contribution7d ?? 0), bossAttacks: Number(edit?.bossAttacks ?? row.bossAttacks ?? 0), lastActivityDays: Number(edit?.lastActivityDays ?? row.lastActivityDays ?? 0), [field]: Number(value) } })); return <tr key={`${row.playerId}-${index}`}><td>{row.name ?? "Unmatched member"}</td><td><InlineEditor value={edit?.power ?? row.power} onChange={(value) => set("power", value)} disabled={!row.playerId} /></td><td><InlineEditor value={edit?.contribution7d ?? row.contribution7d} onChange={(value) => set("contribution7d", value)} disabled={!row.playerId} /></td><td><InlineEditor value={edit?.bossAttacks ?? row.bossAttacks} onChange={(value) => set("bossAttacks", value)} disabled={!row.playerId} /></td><td><InlineEditor value={edit?.lastActivityDays ?? row.lastActivityDays} onChange={(value) => set("lastActivityDays", value)} disabled={!row.playerId} /></td></tr>; }) : bossRows.map((row, index) => { const edit = bossEdits[row.resultId]; const set = (field, value) => row.resultId && setBossEdits((current) => ({ ...current, [row.resultId]: { resultId: row.resultId, bossDamageToday: Number(edit?.bossDamageToday ?? row.bossDamageToday ?? 0), bossRank: Number(edit?.bossRank ?? row.bossRank ?? 1), [field]: Number(value) } })); return <tr key={`${row.resultId ?? row.playerId ?? row.name}-${index}`}><td>{row.name ?? "Unmatched member"}</td><td><InlineEditor value={edit?.bossDamageToday ?? row.bossDamageToday} onChange={(value) => set("bossDamageToday", value)} disabled={!row.resultId} /></td><td><InlineEditor value={edit?.bossRank ?? row.bossRank} onChange={(value) => set("bossRank", value)} disabled={!row.resultId} /></td><td>{row.source ?? "Boss capture"}</td></tr>; })}</tbody></table></div>{state.message ? <p className="daily-editor-feedback" role="status">{state.message}</p> : null}</form>}
  </section></div>;
}
...
function InlineEditor({ value, onChange, disabled }) {
  return <input className="daily-editor-inline" inputMode="numeric" value={value ?? ""} disabled={disabled} onChange={(event) => onChange(event.target.value.replace(/\D/g, ""))} />;
}
