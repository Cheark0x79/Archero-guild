...
from archero_guild.storage.persistence import (
...
class StoragePersistenceTests(unittest.TestCase):
    ...
    def test_persist_import_if_configured_skips_unavailable_database(self) -> None:
        with (
            patch("archero_guild.storage.persistence.persist_import_report", side_effect=RuntimeError("connection refused")),
            patch("archero_guild.storage.persistence._is_database_unavailable", return_value=True),
        ):
...
def _import_report():
    from archero_guild.import_capture import ImportReport
