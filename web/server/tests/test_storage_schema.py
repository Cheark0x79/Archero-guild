...
from archero_guild.storage.export_json import _jsonable, _member_snapshot_row


SCHEMA_PATH = Path(__file__).resolve().parents[3] / "ocr" / "app" / "archero_guild" / "storage" / "schema.sql"
...
class StorageSchemaTests(unittest.TestCase):
    ...
    def test_member_export_prefers_a_detected_chinese_name(self) -> None:
        row = {
            ...
            "raw_payload": {
                "name": "測試玩家",
                "raw_name": "測試玩家 | Bh Bh 28 FA .",
            },
        }

        self.assertEqual(_member_snapshot_row(row)["detectedName"], "測試玩家")
