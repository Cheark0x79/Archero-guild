...
from archero_guild.storage.migrate_json import load_import_report, migrate_import_reports
from ocr.tests.test_import_capture import SAMPLE_DATA
...
class StorageMigrateJsonTests(unittest.TestCase):
    ...
    def test_migrate_import_reports_dry_run_extracts_existing_reports_without_db(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.storage.migrate_json.extract_member_metrics_from_screenshots", return_value=[member_metric]),
                patch("archero_guild.storage.migrate_json.extract_boss_rankings_from_screenshots", return_value=[boss_ranking]),
                patch(
                    "archero_guild.storage.migrate_json.validate_extracted_import",
                    return_value={"status": "accepted", "warnings": [], "errors": []},
                ) as validate,
                patch("archero_guild.storage.migrate_json.persist_import_report") as persist,
            ):
    ...
    def test_migrate_import_reports_skips_ocr_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("archero_guild.storage.migrate_json.extract_member_metrics_from_screenshots") as extract:
