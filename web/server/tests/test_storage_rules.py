...
from archero_guild.storage.rules import _read_legacy_rules, read_rules
...
class StorageRulesTests(unittest.TestCase):
    ...
    def test_partial_database_rules_are_completed_from_legacy_values(self) -> None:
        ...
        with (
            patch("archero_guild.storage.rules._read_database_rules", return_value={"memberCapacity": 35}),
            patch("archero_guild.storage.rules._read_legacy_rules", return_value=legacy),
            patch("archero_guild.storage.rules.write_rules", side_effect=lambda _dsn, rules: rules) as write,
        ):
    ...
    def test_partial_database_rules_remain_explicit_when_no_legacy_file_exists(self) -> None:
        ...
        with (
            patch("archero_guild.storage.rules._read_database_rules", return_value=partial),
            patch("archero_guild.storage.rules._read_legacy_rules", return_value={}),
        ):
