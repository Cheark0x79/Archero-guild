...
from archero_guild.pipeline.guild_member_ocr import (
    RosterEntry,
...
from archero_guild.pipeline.guild_members import Rect
...
class GuildMemberOcrTests(unittest.TestCase):
    ...
    def test_donation_crops_exclude_the_icon_and_trailing_bar(self) -> None:
        ...
        with patch(
            "archero_guild.pipeline.guild_member_ocr._read_integer_candidates",
            return_value=[550],
        ):
    ...
    def test_match_roster_name_removes_ui_prefixes_and_normalizes_zero(self) -> None:
        roster = [
            RosterEntry("1", "Nova00"),
            RosterEntry("2", "OrionX"),
        ]

        self.assertEqual(_match_roster_name(["Members+ NovaOO"], roster)[0].player_id, "1")
        self.assertEqual(_match_roster_name(["bers? orionx"], roster)[0].player_id, "2")

    def test_match_roster_name_uses_confirmed_alias_but_returns_canonical_name(self) -> None:
        roster = [RosterEntry("900000101", "Auroria", aliases=("Aur0ria",))]

        match = _match_roster_name(["Aur0ria"], roster)

        self.assertIsNotNone(match)
        self.assertEqual(match[0].player_id, "900000101")
        self.assertEqual(match[0].name, "Auroria")

    def test_matches_a_cyrillic_member_name(self) -> None:
        roster = [RosterEntry("900000102", "Искатель")]

        match = _match_roster_name(["ИСКАТЕЛЬ |", "UCKATEJIb"], roster)

        self.assertIsNotNone(match)
        self.assertEqual(match[0].player_id, "900000102")
        self.assertEqual(match[2], "ИСКАТЕЛЬ |")

    def test_matches_a_traditional_chinese_member_name(self) -> None:
        roster = [RosterEntry("900000103", "測試玩家")]

        match = _match_roster_name(["測試玩家"], roster)

        self.assertIsNotNone(match)
        self.assertEqual(match[0].player_id, "900000103")
    ...
    def test_clean_observed_name_removes_ui_and_boundary_punctuation(self) -> None:
        self.assertEqual(_clean_observed_name("MapleFox ."), "MapleFox")
        self.assertEqual(_clean_observed_name("Guild Members+ Ceddie12 |"), "Ceddie12")
        self.assertEqual(_clean_observed_name("bers? PixelA |"), "PixelA")
