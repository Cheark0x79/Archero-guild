...
class RemoteOcrTests(unittest.TestCase):
    def test_builds_roster_entries_from_remote_member_payload(self) -> None:
        ...
        self.assertEqual(roster[0].player_id, "900000103")
        self.assertEqual(roster[0].name, "測試玩家")
    ...
    def test_validation_is_the_safe_default_before_publication(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.ocr.remote.fetch_roster") as fetch_roster,
                patch("archero_guild.ocr.remote.build_day_batch", return_value=batch),
                patch(
                    "archero_guild.ocr.remote.publish_batch",
    ...
    def test_publication_reuses_the_reviewed_outbox_without_running_ocr(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.ocr.remote.fetch_roster") as fetch_roster,
                patch("archero_guild.ocr.remote.build_day_batch") as build_day_batch,
                patch(
                    "archero_guild.ocr.remote.publish_batch",
