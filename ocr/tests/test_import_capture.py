...
from archero_guild.import_capture import (
    ImportedScreenshot,
...
export const memberSnapshots = [
  screenshotMember("900000110", "member", 1300000, 2000, 1, 0, "old source"),
];
"""
...
export const dailyRawSnapshots = [
  {
    date: "2026-07-17",
    rows: [
    rawSnapshotMember("900000110", "member", 1340000, 2750, 2, 0, "members-004.png row 6", "2026-07-17"),
    ],
  },
];
...
class ImportCaptureTests(unittest.TestCase):
    def test_capture_paths_reads_legacy_and_uploaded_layouts(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (day_dir / "guild" / "members-002.png").write_bytes(b"uploaded")
...
"""
    ...
    def test_database_failure_restores_dashboard_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", return_value=[object()]),
                patch("archero_guild.import_capture.extract_member_metrics_from_screenshots", return_value=[metric]),
                patch("archero_guild.storage.persistence.persist_import_if_configured", side_effect=RuntimeError("database failed")),
    ...
    def test_validation_failure_never_updates_front_or_database(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", return_value=[object()]),
                patch("archero_guild.import_capture.extract_member_metrics_from_screenshots", return_value=[]),
                patch(
                    "archero_guild.import_capture.validate_extracted_import",
                    side_effect=ImportValidationError("incomplete capture"),
                ),
                patch("archero_guild.import_capture.update_sample_data") as update_front,
                patch("archero_guild.storage.persistence.persist_import_if_configured") as persist,
    ...
    def test_import_writes_report_and_updates_front_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", side_effect=[[object()] * 7, [object()] * 6]),
                patch("archero_guild.import_capture.detect_boss_ranking_rows", side_effect=[[object()] * 8]),
                patch("archero_guild.import_capture.extract_boss_rankings_from_screenshots", return_value=[]),
                patch("archero_guild.import_capture.validate_extracted_import", return_value={"status": "accepted", "warnings": [], "errors": []}),
                patch(
                    "archero_guild.import_capture.extract_member_metrics_from_screenshots",
                    return_value=[
                        ExtractedMemberMetrics(
                            player_id="900000110",
                            name="5m4",
        ...
        self.assertIn('screenshotMember("900000110", "member", 1320000, 2070, 2, 0, "guild-members-001.png row 0", "2026-07-16")', content)
        ...
        self.assertIn('rawSnapshotMember("900000110", "member", 1320000, 2070, 2, 0, "guild-members-001.png row 0", "2026-07-16")', content)
    ...
    def test_import_inserts_last_imported_at_when_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("archero_guild.import_capture.detect_boss_ranking_rows", return_value=[]):
                import_capture_day(
                    "2026-07-16",
                    raw_root=root / "screenshots" / "raw",
                    imports_root=root / "data" / "imports",
    ...
    def test_import_reads_nested_capture_layout(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", return_value=[object()]),
                patch("archero_guild.import_capture.detect_boss_ranking_rows", return_value=[object(), object()]),
                patch("archero_guild.import_capture.extract_member_metrics_from_screenshots", return_value=[]),
                patch("archero_guild.import_capture.extract_boss_rankings_from_screenshots", return_value=[]),
                patch("archero_guild.import_capture.validate_extracted_import", return_value={"status": "accepted", "warnings": [], "errors": []}),
            ):
    ...
    def test_import_preserves_daily_snapshots_for_other_dates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", return_value=[object()]),
                patch("archero_guild.import_capture.detect_boss_ranking_rows", return_value=[object()]),
                patch("archero_guild.import_capture.extract_boss_rankings_from_screenshots", return_value=[]),
                patch("archero_guild.import_capture.validate_extracted_import", return_value={"status": "accepted", "warnings": [], "errors": []}),
                patch(
                    "archero_guild.import_capture.extract_member_metrics_from_screenshots",
        ...
        self.assertIn('rawSnapshotMember("900000110", "member", 1320000, 2070, 2, 0, "guild/members-001.png row 0", "2026-07-16")', content)
        self.assertIn('rawSnapshotMember("900000110", "member", 1340000, 2750, 2, 0, "members-004.png row 6", "2026-07-17")', content)
    ...
    def test_import_does_not_reprocess_older_daily_member_history(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch("archero_guild.import_capture.detect_member_rows", return_value=[object()]),
                patch("archero_guild.import_capture.extract_member_metrics_from_screenshots", side_effect=[current_metrics, previous_metrics]) as extract,
            ):
                import_capture_day(
                    "2026-07-16",
                    raw_root=root / "screenshots" / "raw",
