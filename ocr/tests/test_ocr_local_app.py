...
from archero_guild.ocr.local_app import (
    LocalOcrError,
    LocalOcrHandler,
    build_demo_batch,
...
class LocalOcrAppTests(unittest.TestCase):
    ...
    def test_synchronized_roster_wins_over_old_reviewed_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            remote_roster = [
                RosterEntry(player_id="old-id", name="PixelA", power_hint=None),
                RosterEntry(player_id="900000104", name="MapleFox", power_hint=1670000),
            ]
        ...
        self.assertEqual(
            ...
            [("old-id", "PixelA"), ("900000104", "MapleFox")],
        )
    ...
    def test_confirmed_ocr_alias_is_private_and_reused_by_roster(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            learned = save_identity_alias(
                path,
                observed_name="Members? Aur0ria | Aur0ria",
                player_id="900000101",
                canonical_name="Auroria",
            )
            roster = roster_with_identity_aliases(
                [RosterEntry("900000101", "Auroria")],
                path,
            )
            stored = load_identity_aliases(path)

        self.assertEqual(learned, 2)
        self.assertEqual(roster[0].aliases, ("Members? Aur0ria", "Aur0ria"))
        self.assertTrue(all(item["playerId"] == "900000101" for item in stored))
    ...
    def test_stale_or_missing_ids_are_offered_for_relinking(self) -> None:
        roster = [
            RosterEntry("current-a", "Auroria"),
            RosterEntry("current-b", "PixelA"),
        ]
        batch = {
            "members": [
                {"playerId": "old-a", "rawName": "Aur0ria", "powerText": "1.42M"},
    ...
    def test_roster_cache_is_private_and_round_trips_without_tokens(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            roster = [
                RosterEntry(player_id="900000105", name="PixelA", power_hint=834600),
                RosterEntry(player_id="900000104", name="MapleFox", power_hint=None),
            ]
    ...
    def test_local_scan_uses_cache_without_contacting_a_remote_destination(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("archero_guild.ocr.local_app.fetch_roster", side_effect=AssertionError("unexpected network call")):
                roster, source = handler._scan_roster()

        self.assertEqual(roster[0].player_id, "900000105")
