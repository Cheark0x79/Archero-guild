...
from archero_guild.pipeline.guild_boss import (
...
from archero_guild.pipeline.guild_member_ocr import ExtractedMemberMetrics, RosterEntry, extract_member_metrics_from_screenshots
from archero_guild.pipeline.guild_members import detect_member_rows
from archero_guild.pipeline.image_geometry import ImageGeometry, image_geometry_for_path


_DATE_PATTERN = re.compile(r"^\d{4}-\d{2}-\d{2}$")
_LEGACY_CAPTURE_PATTERN = re.compile(r"^[a-z0-9-]+-(?P<index>\d{3})\.png$")
_NESTED_CAPTURE_PATTERN = re.compile(r"^[a-z0-9-]+-(?P<index>\d{3})\.png$")
_CAPTURE_LAYOUT = {
...
def _import_capture_day_unlocked(
    ...
    *,
    ...
    from archero_guild.storage.persistence import persist_import_if_configured
...
def capture_paths(day_dir: Path, kind: str) -> list[Path]:
    """Return manually uploaded screenshots in legacy or current layout order."""
    if kind not in _CAPTURE_LAYOUT:
        raise ValueError(f"unknown capture kind: {kind}")
    if not day_dir.exists():
        return []

    subdirectory, prefix = _CAPTURE_LAYOUT[kind]
    legacy = [
        path
        for path in day_dir.glob(f"{kind}-*.png")
        if path.is_file() and _LEGACY_CAPTURE_PATTERN.fullmatch(path.name)
    ]
    nested = [
        path
        for path in (day_dir / subdirectory).glob(f"{prefix}-*.png")
        if path.is_file() and _NESTED_CAPTURE_PATTERN.fullmatch(path.name)
    ]
    return sorted([*legacy, *nested], key=lambda path: (int(path.stem.rsplit("-", 1)[1]), str(path)))
